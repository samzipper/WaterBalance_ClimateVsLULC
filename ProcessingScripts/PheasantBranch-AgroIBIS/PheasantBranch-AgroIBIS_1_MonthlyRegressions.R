...
for (flux.name in flux.name.all){
  ...
  file.rename(paste0(git.dir, "Data/PheasantBranch-AgroIBIS/", files), paste0(git.dir, "Data/PheasantBranch-AgroIBIS/PCR_Output/", flux.name, "_", files))
  
  
}  # end of flux.name loop

# save output csv
write.csv(df.out, paste0(LULC, climate, "_MonthlyRegressions_OutputAll.csv"), row.names=F, quote=F)
