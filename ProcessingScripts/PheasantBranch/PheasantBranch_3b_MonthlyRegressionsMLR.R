## PheasantBranch_3a_MonthlyRegressionsMLR.R
# This script is intended to read in monthly discharge and meteorological data for the Pheasant Branch USGS gauge
# and fit a multiple linear regression (MLR). The MLR is calculated using only data from a historical 
# baseline period (1974-1995) and applied to the entire timeseries (1974-2016). The MLR is fit 250 times using
  ...
  for (flux.name in flux.name.all){
    for (mo in mo.list){
      ...
      df.out.mo <- CalculateClimateMLR(df=df, mo=mo, flux.name=flux.name, var.options=var.options,
                                       ...
                                       p.thres=p.thres, neg.allowed=F, n.vars.keep=n.vars.keep, write.vars.keep=T, write.perm=F)
      
      # this can spit out a warning: 'prediction from a rank-deficient fit may be misleading'
      # which occurs when there is collinearity between variables
    ...
    # move CSV output from CalculateClimateMLR script
    files <- list.files(paste0(git.dir, "Data/PheasantBranch/"), pattern="GHCN_MonthlyRegressions_MLR_.")
    file.rename(paste0(git.dir, "Data/PheasantBranch/", files), paste0(git.dir, "Data/PheasantBranch/", flux.name, "/", yr.baseline.end, "/", files))
    
    # save output csv
    write.csv(df.out, paste0(flux.name, "/", yr.baseline.end, "/GHCN_MonthlyRegressions_MLR_OutputAll.csv"), row.names=F, quote=F)
    
    # fit metric
    RMSE(subset(df.out, group=="val")$MLR, subset(df.out, group=="val")$flux)
    NRMSE(subset(df.out, group=="val")$MLR, subset(df.out, group=="val")$flux)
