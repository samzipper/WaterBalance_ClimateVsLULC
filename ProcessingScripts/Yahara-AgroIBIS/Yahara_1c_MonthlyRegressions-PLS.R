## Yahara_1c_MonthlyRegressions-PLS.R
...
# partial least squares regression. A permutation-based approach is used to provide independent
...
# build PLS relationships; set equal to yr.baseline.start-1
...
for (LULC in scen.LULC.all){
  for (climate in scen.climate.all){
    ...
    for (flux.name in flux.name.all){
      for (mo in mo.all){
        ...
        df.out.mo <- CalculateClimatePLS(df=df, mo=mo, flux.name=flux.name, var.options=var.options,
                                         ...
                                         p.thres=p.thres, neg.allowed=neg.allowed, min.PCs.keep=min.PCs.keep, write.vars.keep=T, write.perm=F)
    ...
    # save output csv
    if (hist.LULC){
      write.csv(df.out.pred, paste0("HistLULC_AllClim_", LULC, climate, "_MonthlyRegressions_PLS_OutputAll.csv"), row.names=F, quote=F)
      if (hist.csv){
        write.csv(df.out.hist, "HistLULC_AllClim_CalVal_MonthlyRegressions_PLS_OutputAll.csv", row.names=F, quote=F)
        hist.csv <- F   # disable
        write.vars.keep <- F
        write.PC.keep <- F
      }
    } else {
      write.csv(df.out.pred, paste0(LULC, climate, "_MonthlyRegressions_PLS_OutputAll.csv"), row.names=F, quote=F)
      if (hist.csv){
        write.csv(df.out.hist, "CalVal_MonthlyRegressions_PLS_OutputAll.csv", row.names=F, quote=F)
        hist.csv <- F   # disable
        write.vars.keep <- F
        write.PC.keep <- F
      }
