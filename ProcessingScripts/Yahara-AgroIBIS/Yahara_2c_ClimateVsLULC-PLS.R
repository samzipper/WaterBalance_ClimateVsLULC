...
if (hist.LULC){
  df.baseline <- read.csv("HistLULC_AllClim_CalVal_MonthlyRegressions_PLS_OutputAll.csv", stringsAsFactors=F)
} else {
  df.baseline <- read.csv("CalVal_MonthlyRegressions_PLS_OutputAll.csv", stringsAsFactors=F)
}
...
for (LULC in scen.LULC.all){
  for (climate in scen.climate.all){
    ...
    # read in data
    if (hist.LULC){
      df <- read.csv(paste0("HistLULC_AllClim_", LULC, climate, "_MonthlyRegressions_PLS_OutputAll.csv"), stringsAsFactors=F)
    } else {
      df <- read.csv(paste0(LULC, climate, "_MonthlyRegressions_PLS_OutputAll.csv"), stringsAsFactors=F)
    }
    ...
    df.perm.ann <- dplyr::summarize(group_by(df, flux.name, year, perm),
                                    ...
                                    PLS.baseline.mm = sum(PLS.baseline.mean))
    ...
    df.perm.ann$change.climate <- df.perm.ann$PLS.mm - df.perm.ann$flux.baseline.mm
...
# write output
if (hist.LULC){
  write.csv(df.out, "HistLULC_AllClim_PLS_ClimateVsLULC.csv", row.names=F, quote=F)
} else {
  write.csv(df.out, "PLS_ClimateVsLULC.csv", row.names=F, quote=F)
}
