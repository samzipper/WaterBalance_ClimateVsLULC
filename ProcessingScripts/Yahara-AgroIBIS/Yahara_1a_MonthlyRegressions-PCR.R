## Yahara_1a_MonthlyRegressions-PCR.R
...
# load packages
source("C:/Users/Sam/Dropbox/Work/R/PCA_Functions.R")
...
# build PCR relationships; set equal to yr.baseline.start-1
  ...
  for (climate in scen.climate.all){
    ...
    for (flux.name in flux.name.all){
      for (mo in mo.all){
        ...
        # run PCR script
        df.out.mo <- CalculateClimatePCR(df=df, mo=mo, flux.name=flux.name, 
                                         var.options=var.options,
                                         yr.baseline.start=yr.baseline.start, 
                                         yr.baseline.end=yr.baseline.end, 
                                         n.val.yr=n.val.yr,
                                         p.thres=p.thres, 
                                         cum.var=cum.var, 
                                         min.var=min.var, 
                                         n.perm=n.perm,
                                         neg.allowed=neg.allowed, 
                                         write.vars.keep=write.vars.keep, 
                                         write.PC.keep=write.PC.keep, 
      ...
      # rename output files from CalculateClimatePCR script
      files <- list.files(paste0(git.dir, "Data/Yahara-AgroIBIS/"), pattern="GHCN_MonthlyRegressions_.")
      file.rename(paste0(git.dir, "Data/Yahara-AgroIBIS/", files), paste0(git.dir, "Data/Yahara-AgroIBIS/PCR_Output/", flux.name, "_", files))
    ...
    # save output csv
    if (hist.LULC){
      write.csv(df.out.pred, paste0("HistLULC_AllClim_", LULC, climate, "_MonthlyRegressions_OutputAll.csv"), row.names=F, quote=F)
      if (hist.csv){
        write.csv(df.out.hist, "HistLULC_AllClim_CalVal_MonthlyRegressions_OutputAll.csv", row.names=F, quote=F)
        hist.csv <- F   # disable
        write.vars.keep <- F
        write.PC.keep <- F
      }
    } else {
      write.csv(df.out.pred, paste0(LULC, climate, "_MonthlyRegressions_OutputAll.csv"), row.names=F, quote=F)
      if (hist.csv){
        write.csv(df.out.hist, "CalVal_MonthlyRegressions_OutputAll.csv", row.names=F, quote=F)
        hist.csv <- F   # disable
        write.vars.keep <- F
        write.PC.keep <- F
      }
