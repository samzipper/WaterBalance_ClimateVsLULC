...
method <- "PLS"
if (method=="PCR"){
  df.out <- read.csv(paste0(git.dir, "Data/PheasantBranch/", flux.name, "/GHCN_SensitivityBaseline_PCR_OutputAll.csv"), stringsAsFactors=F)
  colnames(df.out)[colnames(df.out)=="PCR"] <- "stat"
} else if (method=="PLS") {
  df.out <- read.csv(paste0(git.dir, "Data/PheasantBranch/", flux.name, "/GHCN_SensitivityBaseline_PLS_OutputAll.csv"), stringsAsFactors=F)
  colnames(df.out)[colnames(df.out)=="PLS"] <- "stat"
}
...
df.LULCvClimate$change.climate <- df.LULCvClimate$stat.sum - df.LULCvClimate$obs.baseline.mean
