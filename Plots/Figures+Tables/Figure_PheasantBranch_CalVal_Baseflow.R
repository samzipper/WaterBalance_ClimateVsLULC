## Figure_PheasantBranch_CalVal_Baseflow.R
...
p.val.time <- 
  ...
  scale_y_continuous(name="Baseflow [mm]", limits=c(min(df.val.melt$value), max(df.val.melt$value))) +
...
p.climate.LULC.hist <-
  ...
  scale_x_continuous(name="Change in Annual Baseflow Depth [mm]", expand=c(0,0)) +
...
## save plots
pdf(file=paste0(path.fig, "Figure_PheasantBranch_CalVal_Baseflow_NoText.pdf"), width=(181/25.4), height=(120/25.4))
grid.arrange(
